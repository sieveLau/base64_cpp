...
template <int N> auto b64encode(byte (&bytes)[N]) -> std::string {
...
auto b64encode(const std::array<byte, SIZE> &bytes) -> std::string {
  ...
  Base64encode(base64_string.get(), reinterpret_cast<const char *>(bytes),
               SIZE);
  return std::string(base64_string.get());
}

auto b64decode(const std::string& encoded_str, size_t* size=nullptr)-> byte*;
